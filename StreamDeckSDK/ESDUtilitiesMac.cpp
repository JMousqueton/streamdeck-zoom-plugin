...
static std::string CFStringGetStdString(
  ...
  while (range.length > 0) {
    ...
    CFIndex produced;
    CFIndex consumed = CFStringGetBytes(
      inStringRef, range, inEncoding, 0, false, buffer, sizeof(buffer),
      &produced);
    if (consumed == 0)
      return "";
...
std::string ESDUtilities::GetFolderPath(const std::string& inPath) {
  ...
  while (pathWithoutTrailingDelimiters.length() > delimiterString.length()
    ...
    pathWithoutTrailingDelimiters = pathWithoutTrailingDelimiters.substr(
      0, pathWithoutTrailingDelimiters.length() - delimiterString.length());
  }
  ...
  if (std::string::npos != pos) {
    ...
    while (foundPathWithoutTrailingDelimiters.length()
             > delimiterString.length()
           && HasSuffix(foundPathWithoutTrailingDelimiters, delimiterString)) {
      foundPathWithoutTrailingDelimiters
        = foundPathWithoutTrailingDelimiters.substr(
          0, foundPathWithoutTrailingDelimiters.length()
               - delimiterString.length());
    }
  ...
  if (sPluginPath.empty()) {
    ...
    if (bundleRef != NULL) {
      ...
      if (executableURL != NULL) {
        CFURLRef checkURL = CFURLCreateCopyDeletingLastPathComponent(
          kCFAllocatorDefault, executableURL);
        while (checkURL != NULL) {
          ...
          if (pathExtension != NULL) {
            if (
              CFStringCompare(pathExtension, CFSTR("sdPlugin"), 0)
              == kCFCompareEqualTo) {
              CFStringRef path
                = CFURLCopyFileSystemPath(checkURL, kCFURLPOSIXPathStyle);
              if (path != NULL) {
          ...
          checkURL = CFURLCreateCopyDeletingLastPathComponent(
            kCFAllocatorDefault, previousURL);
